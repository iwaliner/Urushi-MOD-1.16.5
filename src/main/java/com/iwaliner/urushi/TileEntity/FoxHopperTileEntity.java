...
public class FoxHopperTileEntity extends LockableLootTileEntity implements IFoxHopper, ITickableTileEntity {
    ...
    public FoxHopperTileEntity(TileEntityType<?> p_i48289_1_) {
        super(p_i48289_1_);
    }
    ...
    private boolean tryMoveItems(Supplier<Boolean> p_200109_1_) {
        if (this.level != null && !this.level.isClientSide) {
            if (!this.isOnCooldown() && this.getBlockState().getValue(FoxHopperBlock.ENABLED)) {
                boolean flag = false;
                if (!this.isEmpty()) {
    ...
    private boolean ejectItems() {
        if (FoxHopperItemHandler.insertHook(this)) return true;
        IInventory iinventory = this.getAttachedContainer();
        if (iinventory == null) {
            return false;
        } else {
            Direction direction = this.getBlockState().getValue(FoxHopperBlock.FACING).getOpposite();
    ...
    public static boolean suckInItems(IFoxHopper p_145891_0_) {
    ...
    private static boolean tryTakeInItemFromSlot(IFoxHopper p_174915_0_, IInventory p_174915_1_, int p_174915_2_, Direction p_174915_3_) {
    ...
    private static ItemStack tryMoveInItem(@Nullable IInventory p_174916_0_, IInventory p_174916_1_, ItemStack p_174916_2_, int p_174916_3_, @Nullable Direction p_174916_4_) {
        ...
        if (canPlaceItemInContainer(p_174916_1_, p_174916_2_, p_174916_3_, p_174916_4_)) {
            ...
            if (flag) {
                if (flag1 && p_174916_1_ instanceof FoxHopperTileEntity) {
                    FoxHopperTileEntity hoppertileentity1 = (FoxHopperTileEntity)p_174916_1_;
                    if (!hoppertileentity1.isOnCustomCooldown()) {
                        int k = 0;
                        if (p_174916_0_ instanceof FoxHopperTileEntity) {
                            FoxHopperTileEntity hoppertileentity = (FoxHopperTileEntity)p_174916_0_;
                            if (hoppertileentity1.tickedGameTime >= hoppertileentity.tickedGameTime) {
                                k = 1;
                            }
                        }

                        hoppertileentity1.setCooldown( hoppertileentity1.tickSend- k);
                    }
                }
    ...
    private IInventory getAttachedContainer() {
        Direction direction = this.getBlockState().getValue(FoxHopperBlock.FACING);
        return getContainerAt(this.getLevel(), this.worldPosition.relative(direction));
    }

    @Nullable
    public static IInventory getSourceContainer(IFoxHopper p_145884_0_) {
        return getContainerAt(p_145884_0_.getLevel(), p_145884_0_.getLevelX(), p_145884_0_.getLevelY() + 1.0D, p_145884_0_.getLevelZ());
    }

    public static List<ItemEntity> getItemsAtAndAbove(IFoxHopper p_200115_0_) {
