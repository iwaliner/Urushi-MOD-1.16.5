...
public class ReiryokuTankTier1Block extends AbstractHasReiryokuBlock {
    public static final DirectionProperty FACING = BlockStateProperties.HORIZONTAL_FACING;
    private static final VoxelShape box = Block.box(1D, 0D, 1D, 15D, 16D, 15D);

    public ReiryokuTankTier1Block(Properties p_i49996_1_) {
        super(p_i49996_1_);
    ...
    public TileEntity newBlockEntity(IBlockReader p_196283_1_) {
        return new ReiryokuTankTier1TileEntity();
    }
    ...
    public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult result) {
        if(!UrushiUtils.isReiryokuStack(player.getItemInHand(hand))){
            if(world.getBlockEntity(pos)instanceof ReiryokuTankTier1TileEntity) {
                ReiryokuTankTier1TileEntity  tileEntity= (ReiryokuTankTier1TileEntity) world.getBlockEntity(pos);

                player.displayClientMessage(new TranslationTextComponent("stored Reiryoku"+" : "+tileEntity.getStoredReiryoku()+" / "+tileEntity.getMaxReiryoku()).withStyle(TextFormatting.WHITE),true);
